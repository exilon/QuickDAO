...
begin
  try
    ...
    DAODatabase.Connection.Server := 'localhost';
    DAODatabase.Connection.Database := 'Test';
    DAODatabase.Connection.UserName := 'myuser';
    DAODatabase.Connection.Password := '1234';
    ...
    crono.Stop;
    coutFmt('Elapsed %s to insert %d records',[crono.ElapsedTime,NumUsers],etSuccess);
    ...
    iresult := DAODatabase.From<TUser>.Where('Age > ? AND Age < ?',[30,35]).OrderBy('Name,Surname').SelectTop(10);
